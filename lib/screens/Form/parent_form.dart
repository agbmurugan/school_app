...
class _ParentFormState extends State<ParentForm> {
  ...
  void initState() {
    ...
    controller = widget.parent == null
        ? ParentFormController()
        : ParentFormController.fromParent(widget.parent!);
    getChildern();
    super.initState();
  }
  ...
  Future<void> getChildern() async {
    if (widget.parent != null) {
      ...
      await firestore
          .collection('students')
          .where('parents', arrayContains: widget.parent!.icNumber)
          .get()
          .then((value) {
        children =
            value.docs.map((e) => Student.fromJson(e.data(), e.id)).toList();
      });
                ...
                onPressed: () {
                  showDialog(
                      ...
                      builder: (context) {
                        return Dialog(
                            insetPadding:
                                EdgeInsets.all(isMobile(context) ? 32 : 300),
                            child: FutureBuilder(
                              future: getChildern(),
                            ...
                            child: ElevatedButton(
                                onPressed: () async {
                                  if (controller.fileData != null) {
                                    controller.image = await uploadImage(
                                        controller.fileData!,
